...
let nextJoke = () => {
  let randomJoke = Math.floor(Math.random() * 851) - 1;

  let currentJoke = (JokeArray[randomJoke]);
  let jokeBody = document.getElementsByClassName('jokeBody')[0].innerHTML = currentJoke.body;
  if (jokeBody.length > 550) {
    nextJoke();
  }

  let jokeCategory = document.getElementsByClassName('jokeCategory')[0].innerHTML = currentJoke.category;
  let jokeRating = document.getElementsByClassName('jokeRating')[0].innerHTML = currentJoke.rating;
}
nextJoke();
